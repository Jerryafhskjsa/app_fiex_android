...
//        NONE_COLOR = CommonUtil.setColorAlpha(NONE_STEP_COLOR, 0);
        ...
        T21 = SkinCompatResources.getColor(context, R.color.T21)
        T22 = SkinCompatResources.getColor(context, R.color.T22)
...
//            offset += textPaint.measureText(ma10) + TEXT_SPACING;
        ...
        /*run {
            ...
            val vol = "VOL:" + if (volv == 0.0) context.getString(R.string.number_default) else NumberUtil.formatNumberNoGroup(volv, 2, 5)
...
//            offset += textPaint.measureText(vol) + TEXT_SPACING;
            ...
            if (timeStep !== TimeStep.NONE) {
                ...
                val volma5 = "MA5:" + if (volma5v == 0.0) context.getString(R.string.number_default) else NumberUtil.formatNumberNoGroup(volma5v, 2, 5)
...
//            offset += textPaint.measureText(volma5) + TEXT_SPACING;
                ...
                val volma10 = "MA10:" + if (volma10v == 0.0) context.getString(R.string.number_default) else NumberUtil.formatNumberNoGroup(volma10v, 2, 5)
            ...
            if (showingMaxVol != 0.0) {
                val volmax = NumberUtil.formatNumberNoGroup(showingMaxVol, 2, 5)
...
//        }
    ...
    fun addDataList(page: Int, kLineItems: ArrayList<KLineItem?>?) {
        ...
        //返回的数据刚好是后一页数据，添加数据
        var addedData: Array<KLineChartItem?>? = getAllNode(kLineItems, timeStep.value)
        lastPage = page
        if (moreDataCache != null) {
            for (i in lastPage + 1 until moreDataCache!!.size) {
                val cacheData = moreDataCache!![i] ?: break
                val nextPageData = getAllNode(cacheData, timeStep.value)
                addedData = addKLIneItemsToFront(addedData!!, nextPageData, timeStep.value)
        ...
        val oldSize = if (data?.kLineChartItems == null) 0 else data?.kLineChartItems?.size ?: 0
        data?.kLineChartItems = addKLIneItemsToFront(data?.kLineChartItems!!, addedData, timeStep.value)
        run {
            val size = data?.kLineChartItems?.size ?: 0
            //计算增加的节点数据
            calculate(data?.kLineChartItems!!)
...
//            KLineChartItems = newNodes;
    ...
    fun getTimeStepRequestStr():String?{
        ...
        when(timeStep.text.toString()){
            ...
            context.getString(R.string.hour_6) -> str = "6h"
