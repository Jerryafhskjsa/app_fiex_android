...
class PairStatusAdapter(context: Context, data: MutableList<PairStatusShowPopup?>?, private val dataType: Int) : BaseDataTypeAdapter<PairStatusShowPopup?>(context, data) {
    ...
    private fun bindView(convertView: View, pairStatus: PairStatusShowPopup?) {
        ...
        if (dataType == PairStatus.LEVER_DATA) {
            val maxMultiple = if (pairStatus?.leverConfEntity == null) null else pairStatus.leverConfEntity?.maxMultiple
            val max = maxMultiple?.toInt()
            viewHolder.maxLeverView?.visibility = View.VISIBLE
            viewHolder.maxLeverView?.text = String.format("%sX", if (max == null) nullAmount else NumberUtil.formatNumberNoGroup(max))
        } else {
            viewHolder.maxLeverView?.visibility = View.GONE
        }
        if (viewHolder.pairNameView != null) {
            viewHolder.pairNameView?.text = pairStatus?.name
        }
