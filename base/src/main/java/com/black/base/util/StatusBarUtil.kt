...
import com.black.base.util.OSUtils.isFlyme
import com.black.base.util.OSUtils.isMiui
...
object StatusBarUtil {
    ...
    fun setStatusBarColor(activity: Activity, colorId: Int) {
        ...
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) { //使用SystemBarTintManager,需要先将状态栏设置为透明
...
//window.setNavigationBarColor(Color.TRANSPARENT);
    ...
    private fun getActionBarView(activity: Activity): View? {
        if (activity is AppCompatActivity) {
            val actionBar = activity.supportActionBar ?: return null
            return actionBar.customView
        }
        return null
    }

    private fun getToolBar(activity: Activity): Toolbar? {
        if (activity is AppCompatActivity) {
            val actionBar = activity.supportActionBar ?: return null
            val customView = actionBar.customView ?: return null
            val toolbar = customView.parent
    ...
    private fun getWindowView(activity: Activity): View? {
        val contentView = activity.findViewById<View>(R.id.content) as ViewGroup
                ?: return null
        var viewParent: ViewParent? = contentView.parent ?: return null
    ...
    fun fitPopupWindowOverStatusBar(mPopupWindow: PopupWindow?, needFullScreen: Boolean) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            try {
                ...
                mLayoutInScreen[mPopupWindow] = needFullScreen
