...
class ContractOrderHistoryAdapter(context: Context, variableId: Int, data: MutableList<OrderBeanItem>?) : BaseRecycleDataBindAdapter<OrderBeanItem, ListTabContractPlanBinding>(context, variableId, data){
    ...
    override fun onBindViewHolder(
        ...
        viewHolder?.tvRevoke?.visibility = View.GONE
        viewHolder?.shijian?.visibility = View.GONE
        viewHolder?.timer?.visibility = View.VISIBLE
        ...
        viewHolder?.tvDealAmount?.text = String.format(context.getString(R.string.contract_deal_amount), unit)
        viewHolder?.tvProfitPrice?.text = String.format("平仓盈亏(%s)", unit)
        viewHolder?.tvLosePrice?.text = String.format("回报率(%s)", unit)
        viewHolder?.tvBondAmount?.text = String.format("手续费(%s)", unit)
        ...
        //手续费
        viewHolder?.tvDealAmountDes?.text = String.format("%.4f",num2)
        //成交均价
        viewHolder?.tvProfitPriceDes?.text = if (planData.closeProfit == null ) nullAmount else planData.closeProfit
        //成交数量
        viewHolder?.tvLosePriceDes?.text = "--"
        //平仓盈亏
