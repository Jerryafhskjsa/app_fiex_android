...
import com.black.c2c.databinding.FragmentC2cOneKeySaleBinding
import com.google.android.material.tabs.TabLayout
import skin.support.content.res.SkinCompatResources
import java.util.*

class C2COneKeySaleFragment : BaseFragment() {
    private var binding: FragmentC2cOneKeySaleBinding? = null
    private var supportCoins: ArrayList<C2CSupportCoin>? = null
    private var fragmentList: MutableList<C2COneKeySaleItemFragment>? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_c2c_one_key_sale, container, false)
    ...
    private fun refreshCoinTabs() {
        if (binding?.coinTab != null) {
            ...
            if (supportCoins != null && supportCoins!!.isNotEmpty()) {
                val size = supportCoins?.size ?: 0
                for (i in 0 until size) {
                    val c2CSupportCoin = supportCoins!![i]
                    try {
                        val itemFragment = C2COneKeySaleItemFragment()
                        val bundle = Bundle()
            ...
            for (i in 0 until binding?.coinTab?.tabCount!!) {
                val c2CSupportCoin = supportCoins!![i]
