...
import com.black.lib.photoview.Compat.postOnAnimation
import java.lang.ref.WeakReference

open class PhotoViewAttacher(imageView: ImageView) : IPhotoView, OnTouchListener, VersionedGestureDetector.OnGestureListener, OnDoubleTapListener, OnGlobalLayoutListener {
    companion object {
...
// }
    ...
    override fun onFling(startX: Float, startY: Float, velocityX: Float, velocityY: Float) {
        ...
        if (hasDrawable(imageView)) {
            ...
            mCurrentFlingRunnable!!.fling(imageView.width, imageView.height, velocityX.toInt(), velocityY.toInt())
    ...
    override fun onGlobalLayout() {
        ...
        if (null != imageView && mZoomEnabled) {
             ...
             */
            if (top != mIvTop || bottom != mIvBottom || left != mIvLeft || right != mIvRight) { // Update our base matrix, as the bounds have changed
...
// to min scale
    ...
    override fun setMinScale(minScale: Float) { //		checkZoomLevels(minScale, mMidScale, mMaxScale);
...
//		mMidScale = midScale;
//	}
    override fun setMaxScale(maxScale: Float) { //		checkZoomLevels(mMinScale, mMidScale, maxScale);
    ...
    fun update() {
        val imageView: ImageView? = imageView
        if (null != imageView) {
            if (mZoomEnabled) { // Make sure we using MATRIX Scale Type
                setImageViewScaleTypeMatrix(imageView)
    ...
    private fun checkImageViewScaleType() {
        ...
        if (null != imageView && imageView !is PhotoView) {
            check(imageView.scaleType == ImageView.ScaleType.MATRIX) { "The ImageView's ScaleType has been changed since attaching a PhotoViewAttacher" }
        }
    }
    ...
    private fun updateBaseMatrix(d: Drawable?) {
        ...
        if (mScaleType == ImageView.ScaleType.CENTER) {
            ...
            val mTempSrc = RectF(0.toFloat(), 0.toFloat(), drawableWidth.toFloat(), drawableHeight.toFloat())
    ...
    private inner class AnimatedZoomRunnable(currentZoom: Float, private val mTargetZoom: Float, private val mFocalX: Float,
                                             private val mFocalY: Float) : Runnable {
        ...
        override fun run() {
            ...
            if (null != imageView) {
                ...
                if (mDeltaScale > 1f && currentScale < mTargetZoom
                        || mDeltaScale < 1f && mTargetZoom < currentScale) { // We haven't hit our target scale yet, so post ourselves
// again
                    postOnAnimation(imageView, this)
                } else { // We've scaled past our target zoom, so calculate the
// necessary scale so we're back at target zoom
        ...
        val ANIMATION_SCALE_PER_ITERATION_IN = 1.07f
        val ANIMATION_SCALE_PER_ITERATION_OUT = 0.93f
