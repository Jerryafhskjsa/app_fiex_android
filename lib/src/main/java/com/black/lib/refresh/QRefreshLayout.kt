 ...
 */
class QRefreshLayout @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : ViewGroup(context, attrs), SkinCompatSupportable, NestedScrollingParent, NestedScrollingChild {
    companion object {
    ...
    private fun setScrollListener() {
        ...
        if (viewTarget is RecyclerView) {
            (viewTarget as RecyclerView).addOnScrollListener(object : RecyclerView.OnScrollListener() {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                when (dragMode) {
                    N_DOWN -> {
                        ...
                        viewTarget?.translationY = overScroll
                        if (!isRefreshing) {
                            viewRefresh?.setHeight(overScroll, refreshMidHeight.toFloat(), viewContentHeight.toFloat())
                        }
                        if (!isRefreshing) {
                    ...
                    N_UP -> if (checkCanLoadMore()) {
                        ...
                        viewTarget?.translationY = overScroll
                        if (!isLoading) {
                            viewLoad?.setHeight(abs(overScroll), loadMidHeight.toFloat(), viewContentHeight.toFloat())
    ...
    override fun requestDisallowInterceptTouchEvent(b: Boolean) {
        if (Build.VERSION.SDK_INT < 21 && viewTarget is AbsListView || viewTarget != null && !ViewCompat.isNestedScrollingEnabled(viewTarget!!)) {
        } else {
    ...
    private fun animateToSecondFloor() {
        ...
        if (animatorToSecondFloor == null) {
            ...
            animatorToSecondFloor?.addUpdateListener { animation ->
                ...
                viewRefreshContainer?.translationY = overScroll / 2
                viewLoadContainer?.translationY = overScroll / 2
                viewRefresh?.setHeight(abs(overScroll), loadMidHeight.toFloat(), viewContentHeight.toFloat())
                viewTarget?.translationY = overScroll
