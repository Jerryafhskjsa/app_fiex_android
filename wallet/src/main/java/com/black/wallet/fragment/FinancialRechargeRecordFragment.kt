...
class FinancialRechargeRecordFragment : BaseFragment(), OnItemClickListener, QRefreshLayout.OnRefreshListener, QRefreshLayout.OnLoadListener, QRefreshLayout.OnLoadMoreCheckListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        getFinancialRecordRechargeData(true)
        return layout
    }
    ...
    override fun onItemClick(recyclerView: RecyclerView?, view: View, position: Int, item: Any?) {
        ...
        BlackRouter.getInstance()
                .build(RouterConstData.RECHARGE_DETAIL)
                .with(extras)
                .go(mContext)
    }
    ...
    override fun onRefresh() {
        currentPage = 1
        getFinancialRecordRechargeData(false)
    }
    ...
    private fun getFinancialRecordRechargeData(isShowLoading: Boolean) {
        WalletApiServiceHelper.getWalletRecord(mContext, isShowLoading, currentPage, 10, total,  0, wallet?.coinType, object : NormalCallback<HttpRequestResultData<PagingData<FinancialRecord?>?>?>(mContext!!) {
                ...
                if (returnData != null && returnData.code == HttpRequestResult.SUCCESS) {
                    ...
                    if (dataList != null) {
                        for (record in dataList) {
                            record?.actionType = getString(R.string.wallet_bill_deposit)
                        }
                    }
                    showData(dataList)
                } else {
