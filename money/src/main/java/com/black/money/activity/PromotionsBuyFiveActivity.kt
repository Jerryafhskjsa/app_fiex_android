...
import com.black.money.adpter.PromotionsFiveListAdapter
import com.black.money.databinding.ActivityPromotionsBuyFiveBinding
...
@Route(value = [RouterConstData.PROMOTIONS_BUY_FIVE], beforePath = RouterConstData.LOGIN)
class PromotionsBuyFiveActivity : BaseActivity(), View.OnClickListener, QRefreshLayout.OnRefreshListener, OnItemClickListener, OnLoadListener, OnLoadMoreCheckListener, PromotionsFiveListAdapter.OnPromotionsBuyFiveListener {
    private var binding: ActivityPromotionsBuyFiveBinding? = null
    private var adapter: PromotionsFiveListAdapter? = null
    private var currentPage = 1
    private var total = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.activity_promotions_buy_five)
        ...
        adapter = PromotionsFiveListAdapter(this, BR.listItemPromotionsBuyFiveModel, null)
        adapter?.setOnPromotionsBuyFiveListener(this)
    ...
    override fun getTitleText(): String {
        return getString(R.string.purchase_five)
    }
    ...
    private fun getPromotionsBuy(isShowLoading: Boolean) {
        MoneyApiServiceHelper.getPromotionsBuyFiveList(this, 1, currentPage, 8, isShowLoading, object : NormalCallback<HttpRequestResultData<PagingData<PromotionsBuyFive?>?>?>() {
            ...
            override fun callback(returnData: HttpRequestResultData<PagingData<PromotionsBuyFive?>?>?) {
                binding?.refreshLayout?.setRefreshing(false)
                binding?.refreshLayout?.setLoading(false)
                if (returnData != null && returnData.code == HttpRequestResult.SUCCESS) {
                    total = returnData.data?.total ?: 0
                    showPromotionsBuy(returnData.data?.data)
                } else {
    ...
    private fun showPromotionsBuy(list: ArrayList<PromotionsBuyFive?>?) {
    ...
    override fun onBuy(promotions: PromotionsBuyFive?) {
        promotions?.let {
            val bundle = Bundle()
            bundle.putParcelable(ConstData.PROMOTIONS_BUY_FIVE, promotions)
            BlackRouter.getInstance().build(RouterConstData.PROMOTIONS_BUY_FIVE_DETAIL).with(bundle).go(this)
        }
    }
    ...
    private fun refreshCountdown(): Boolean {
        ...
        if (updatePositions.isNotEmpty()) {
            for (i in updatePositions) {
                adapter?.notifyItemChanged(i, PromotionsFiveListAdapter.STATUS)
            }
        }
