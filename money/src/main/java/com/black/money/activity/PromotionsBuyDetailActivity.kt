...
import com.black.money.databinding.ActivityPromotionsBuyDetailBinding
...
@Route(value = [RouterConstData.PROMOTIONS_BUY_DETAIL])
class PromotionsBuyDetailActivity : BaseActivity(), View.OnClickListener, QRefreshLayout.OnRefreshListener {
    private var dm: DisplayMetrics? = null
    private var promotionsBuy: PromotionsBuy? = null

    private var binding: ActivityPromotionsBuyDetailBinding? = null

    private var imageLoader: ImageLoader? = null
    private var promotionsBuyDetail: PromotionsBuyDetail? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_promotions_buy_detail)

        binding?.actionBarTitle?.text = getString(R.string.purchase_title2, promotionsBuy?.coinType)
    ...
    override fun onClick(view: View) {
        ...
        if (id == R.id.promotions_record) {
            val bundle = Bundle()
            bundle.putParcelable(ConstData.PROMOTIONS_BUY, promotionsBuy)
            BlackRouter.getInstance().build(RouterConstData.PROMOTIONS_BUY_RECORD).with(bundle).go(this)
    ...
    private fun getPromotionsBuyDetail(isShowLoading: Boolean) {
        MoneyApiServiceHelper.getPromotionsBuyDetail(this, NumberUtil.formatNumberNoGroup(promotionsBuy?.id), isShowLoading, object : NormalCallback<HttpRequestResultData<PromotionsBuyDetail?>?>(mContext!!) {
            ...
            override fun callback(returnData: HttpRequestResultData<PromotionsBuyDetail?>?) {
                binding?.refreshLayout?.setRefreshing(false)
                if (returnData != null && returnData.code == HttpRequestResult.SUCCESS) {
                    promotionsBuyDetail = returnData.data
                    if (promotionsBuyDetail != null) {
                        promotionsBuyDetail?.purchaseId = NumberUtil.formatNumberNoGroup(promotionsBuy?.id)
                    }
                } else {
                    promotionsBuyDetail = null
    ...
    private fun showPromotionsBuyDetail() {
        ...
        imageLoader?.loadImage(binding?.info, if (promotionsBuyDetail == null) null else UrlConfig.getHost(this) + promotionsBuyDetail?.infoImageUrl, (dm?.density!! * 1.5 * 1024 * 1024).toLong())
        binding?.amount?.text = String.format("%s/%s",
                if (promotionsBuyDetail == null || promotionsBuyDetail?.nowAmount == null) nullAmount else NumberUtil.formatNumberDynamicScaleNoGroup(promotionsBuyDetail?.nowAmount, 8, 0, 0),
                if (promotionsBuyDetail == null || promotionsBuyDetail?.totalAmount == null) nullAmount else NumberUtil.formatNumberDynamicScaleNoGroup(promotionsBuyDetail?.totalAmount, 8, 0, 0))
        binding?.personCount?.text = if (promotionsBuyDetail == null || promotionsBuyDetail?.personNum == null) nullAmount else NumberUtil.formatNumberNoGroupHardScale(promotionsBuyDetail?.personNum, 0)
    ...
    private fun showBuyDialog(promotions: PromotionsBuyDetail?) {
        val contentView = LayoutInflater.from(mContext).inflate(R.layout.dialog_promotions_buy, null)
            ...
            override fun onItemClick(recyclerView: RecyclerView?, view: View, position: Int, item: Any?) {
                ...
                if (amount == null) {
                    ...
                    if (price == null) {
                        ...
                        val amountHint = String.format("（%s %s ≈%s %s）",
                                amountText,
                                if (promotions?.coinType == null) "" else promotions.coinType,
                                NumberUtil.formatNumberDynamicScaleNoGroup(price * amount, 8, 0, 8),
                                if (supportCoinSelected?.coin == null) nullAmount else supportCoinSelected.coin)
                ...
                val priceDouble = CommonUtil.parseDouble(supportCoinSelected?.price)
                val wallet = if (supportCoinSelected == null) null else walletCache[supportCoinSelected.coin]
                val useAmount = if (supportCoinSelected == null) nullAmount else
                    String.format("%s %s",
                            if (wallet?.coinAmount == null || priceDouble == null) nullAmount else NumberUtil.formatNumberNoGroupHardScale(wallet.coinAmount!! / BigDecimal(priceDouble), 2),
                            if (promotions?.coinType == null) nullAmount else promotions.coinType)
        ...
        val price = ((if (supportCoin?.price == null) nullAmount else supportCoin.price)
        ...
        val useAmount = if (supportCoin == null) nullAmount else
            String.format("%s %s",
                    if (wallet?.coinAmount == null || priceDouble == null) nullAmount else NumberUtil.formatNumberNoGroupHardScale(wallet.coinAmount!! / BigDecimal(priceDouble), 2),
                    if (promotions?.coinType == null) nullAmount else promotions.coinType)
        useAmountView.text = useAmount
        ...
        amountView.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(charSequence: CharSequence, i: Int, i1: Int, i2: Int) {
                ...
                if (amount == null) {
                    ...
                    val supportCoinSelected: SupportCoin? = adapter.getItem(adapter.getCheckedIndex())
                    ...
                    if (priceSelected == null) {
                        ...
                        val amountHint = String.format("（%s %s ≈%s %s）",
                                amountText,
                                if (promotions?.coinType == null) "" else promotions.coinType,
                                NumberUtil.formatNumberDynamicScaleNoGroup(priceSelected * amount, 8, 0, 8),
                                if (supportCoinSelected?.coin == null) nullAmount else supportCoinSelected.coin)
                        amountHintView.text = amountHint
                    }
                }
        ...
        coinType02View.text = if (promotions?.coinType == null) "" else promotions.coinType
