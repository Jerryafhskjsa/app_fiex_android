...
import com.black.money.adpter.CloudPowerRewardRecordAdapter
import com.black.money.databinding.FragmentCloudPowerRewardRecordBinding
import com.black.net.HttpRequestResult
import skin.support.content.res.SkinCompatResources
import java.util.*

class CloudPowerRewardFragment : BaseFragment(), QRefreshLayout.OnRefreshListener, OnLoadListener, OnLoadMoreCheckListener {
    private var binding: FragmentCloudPowerRewardRecordBinding? = null
    private var adapter: CloudPowerRewardRecordAdapter? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_cloud_power_reward_record, container, false)
        ...
        adapter = CloudPowerRewardRecordAdapter(mContext!!, BR.listItemCloudPowerRewardRecordModel, null)
    ...
    private fun getCloudPowerHoldRecord(isShowLoading: Boolean) {
        MoneyApiServiceHelper.getCloudPowerRewardRecord(mContext, isShowLoading, currentPage, 10, object : NormalCallback<HttpRequestResultData<PagingData<CloudPowerRewardRecord?>?>?>(mContext!!) {
            override fun error(type: Int, error: Any?) {
                super.error(type, error)
                showDataList(null)
            }

            override fun callback(returnData: HttpRequestResultData<PagingData<CloudPowerRewardRecord?>?>?) {
    ...
    private fun showDataList(data: ArrayList<CloudPowerRewardRecord?>?) {
        if (adapter != null) {
            if (currentPage == 1) {
                adapter?.data = data
            } else {
