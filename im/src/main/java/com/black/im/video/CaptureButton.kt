...
import com.black.im.video.util.CheckPermission.recordState
...
class CaptureButton : View {
    companion object {
        ...
        private val TAG = CaptureButton::class.java.simpleName
    }

    private var state = 0 //当前按钮状态
    private var button_state = 0 //按钮可执行的功能状态（拍照,录制,两者）
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> if (captureLisenter != null && state == STATE_RECORDERING && (button_state == JCameraView.BUTTON_STATE_ONLY_RECORDER || button_state == JCameraView.BUTTON_STATE_BOTH)) { //记录当前Y值与按下时候Y值的差值，调用缩放回调接口
    ...
    private fun handlerUnpressByState() {
        ...
        when (state) {
            STATE_PRESS -> if (captureLisenter != null && (button_state == JCameraView.BUTTON_STATE_ONLY_CAPTURE || button_state ==
